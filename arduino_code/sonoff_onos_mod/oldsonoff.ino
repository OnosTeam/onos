...
unsigned long send_address_timeout=0;
...
void loop()
{
  ...
  Serial.println(getStateText(bitRead(power,1)) ); // 1 is the first device ..so the plug


  if ( (millis()-sync_time)>*send_address_timeout_pointer){   //each n sec time contact the onosCenter and update
    ip_sent_to_onosCenter=sendCurrentIp();
    sync_time=millis();
  }
  
  if (ip_sent_to_onosCenter==0){    //onosmod
    *send_address_timeout_pointer=1000;
  }
  else{
    *send_address_timeout_pointer=5500;
  }
